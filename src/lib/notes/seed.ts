...
- 导入：左上角加号里导入 Markdown、TXT、文件夹或电子书（EPUB、Kindle、FB2、带章节的 TXT、HTML）。电子书出现在侧栏「书」，点书名展开或收起章节，点阅读从上次停下的章节接着看；划线可摘到「摘录」；阅读页可朗读，英文专业词按单词读。齿轮 → 朗读 可选语种、音色，以及读完本章是否继续。点章节仍是 Markdown。侧栏《廊下三章》是示例
- 主题、图床、朗读与关于：侧栏齿轮 → 主题 / 图像 / 朗读 / 关于。宣纸是默认，代码高亮已单独加深。图床可直接填，也可粘贴 PicGo 配置。朗读可选中文、英语或中英混合，并指定音色；读完本章是否继续也可在这里关。关于里有版本、协议和捐赠
- 发布到博客：齿轮 → 博客填 GitHub 或 Gitee 仓库。侧栏报纸图标可查看仓库里已有的文章，点开阅读或拉进本地再改，工具栏纸飞机发回去。新建可用模板
- 公式、任务、双链：\`$E=mc^2$\`，预览里勾选 \`- [ ]\`，\`[[笔记名]]\` 跳转，Ctrl + P 快速打开
- 安卓 / 电脑：在文件管理器里用「打开方式」选静笺，可直接打开 Markdown、TXT、EPUB、Kindle、FB2；也可把文件拖进编辑区
...
电子书可以这样读：侧栏「书」里点书名展开章节，再点阅读会回到上次的章节和位置；收起书名就能翻到下面的笔记。划线后点「摘录」写进笔记；朗读只出现在阅读页，可把 Kubernetes、JSON 这类词按单词读出。读完本章会接着下一章，可在齿轮 → 朗读 关掉自动播放。点章节仍打开 Markdown。加号「电子书」可导入 EPUB、Kindle、FB2、带章节的 TXT 或 HTML。改完再导出本书，会带上作者和封面。`,
    },
  ];
